...
{  
  ...
  for(int i = 0; i < net_.layer().size(); ++i)
  {
    if(std::string(net_.layer(i).type()).compare("InnerProduct") == 0)
      ...
      assert(net_.layer(i).blobs(0).data().size() == (input_size*output_size));

      // append new op 
      ops_.push_back(std::move(std::unique_ptr<Op>(
        new MatVecOp(dev_, weight, bias, input_size, output_size))));
...
const float* DigitDNN::run(const float* input, bool copy)
{
  if(copy)
  {
  ...
  for(int i = 0 ; i < ops_.size(); ++i)
  {
    ...
    if(!copy && pair.first == input_idx_)
    {
      ops_[i]->run(input, data_[pair.second].data());
    }
    else
    {
